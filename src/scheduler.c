...
int myscheduler_start() {
    ...
    // If any thread will end then it will return here, because every thread context is linked to scheduler main
    // context. In case of such situation we remove this thread from scheduler.
...
void scheduler_remove_dead_thread() {
    ...
    if (node_to_remove == NULL) {
        ERROR("Cannot remove dead thread because it does not exists in carousel. Aborting because something went "
              "horribly wrong.");
        abort();
    }
...
void scheduler_make_current_thread_pending() {
    ...
    INFO("Thread with id: %d will be moved from active list to pending list in scheduler.",
...
void scheduler_remove_one_thread_from_pending_list() {
    ...
    INFO("Thread with id: %d will be moved from active list to pending list in scheduler.",
         scheduler.pending_carousel->tail->thread->id);
